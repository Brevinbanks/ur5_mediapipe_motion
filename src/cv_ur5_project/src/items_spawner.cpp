// spawn the red items on the conveyor belt
...
//ros communications:
  ...
  // initialize items speed: /gazebo/apply_body_wrench
  // get urdf file path of items from parameter servicer
  //publish all current items through topic: /current_items
  ...
  int main(int argc, char **argv) {
      ros::init(argc, argv, "item_spawner");
      ros::NodeHandle nh;
      srand(time(0));
      ...
      //publisher for current_items
      ros::Publisher current_items_publisher = nh.advertise<std_msgs::Int8MultiArray>("current_items",1);
      std_msgs::Int8MultiArray current_items_msg;
      current_items_msg.data.clear();
      ...
      //get file path of items from parameter service
      std::string BlueCube_path;
      bool get_red_path;
      get_red_path = nh.getParam("/BlueCube_path", BlueCube_path);

      if (!(get_red_path)){
          return 0;}
          else{ROS_INFO_STREAM(BlueCube_path << " has been extracted");
}

      std::ifstream red_inXml(BlueCube_path.c_str());
      std::stringstream red_strStream;
      std::string red_xmlStr;

      /*red_inXml.open(BlueCube_path.c_str());*/
      red_strStream << red_inXml.rdbuf();
      red_xmlStr = red_strStream.str();
     // ROS_INFO_STREAM("urdf: \n" <<red_xmlStr);
      // prepare the pawn model service message
      spawn_model_req.initial_pose.position.x = 1.2;
      spawn_model_req.initial_pose.position.z = 1.3;
      ...
      if (ros::ok()){
          ...
          model_name = "red_items_" + index;  // initialize model_name
          ...
          // publish current cylinder items status, all cylinder items will be published
          // no matter if it's successfully spawned, or successfully initialized in speed
            
          current_items_publisher.publish(current_items_msg);
          ROS_INFO("Published");
